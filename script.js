      ...
      if (wrapper) {
        wrapper.innerHTML = `
          <div class="swiper-slide !w-full flex flex-col items-center justify-center p-8 lg:p-16 bg-[#EAE8E4] rounded-2xl border-2 border-dashed border-gray-400 text-black">
            ...
            <h3 class="text-2xl lg:text-4xl font-ermilov uppercase text-center mb-4">${homepage.futureEvents?.emptyTitle || 'Нові події готуються'}</h3>
            <p class="text-lg lg:text-2xl text-center opacity-70">${homepage.futureEvents?.emptyDescription || 'Згодом ви тут побачите наші майбутні події. Слідкуйте за оновленнями!'}</p>
          </div>
        `;
      }
    }
...
// Leaflet Map Logic (Removed in favor of ArcGIS StoryMap iframe)
function initMapMarkers() {
  // Map is now loaded via iframe, so no JS initialization is needed
}
